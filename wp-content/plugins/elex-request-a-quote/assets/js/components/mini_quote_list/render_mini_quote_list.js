...
const MiniQuote = (props) => {
    ...
    const ShowListOnHover = (props) => {
        ...
        return(<div
            ...
            <h6 className="text-nowrap text-center">{props.count}  items in Quote List</h6>
            {miniquoteItems.quote_list.items.map((item,i) => <MiniQuoteContents onDelete={() => onQuoteListItemDelete(i)} show_quantity={miniquoteItems.settings.show_on_product_table.quantity} show_price ={miniquoteItems.settings.show_on_product_table.product_price} hide_price_global={miniquoteItems.general.hide_add_to_cart.hide_price } currency={miniquoteItems.quote_list.wc_currency} item={item} key={miniquoteItems.quote_list.items.product_id} />)}
            <a href={props.path}><button  className="elex-raq-view-quote-list-open-btn btn btn-sm btn-primary w-100">View Quote List</button></a>
        </div>)
        
    }

    const [miniquoteItems, setQuoteItems] = wp.element.useState(props.quote_items);
    let count=0;
    ...
    <div className="d-flex gap-3 align-items-center mb-3 py-2">
            ...
            <div className="d-flex justify-content-end ">
                <div className="nav position-relative">
                    <li
                        ...
                        <a href={miniquoteItems.settings.selected_page} ><button  onMouseOver={ChangeIconColor} onMouseOut = {ChangeColorToWhite} className="btn btn-sm p-0 shadow-none position-relative">
                            <svg id="quote_list_svg"  xmlns="http://www.w3.org/2000/svg" width="30" height="26" viewBox="0 0 36.01 31.5">
                                <g id="Quote_list_icon" data-name="Quote list icon" transform="translate(0 -32)">
                        ...
                        <ShowButtonLabel  label={widget.button_label}  show_label={widget.show_button_label} />
                        <ShowListOnHover  count={count} color={widget.widget_color}  account_url = {miniquoteItems.page_url} disable_for_guest={miniquoteItems.general.general.disable_quote_for_guest} path={miniquoteItems.page_url} />
...
jQuery.ajax({
    ...
    success: function(data){
        wp.element.render(
            <MiniQuote widgetSettings={data.data.widget}  quote_items={data.data} key={data.data.id} />, document.getElementById('mini_quote_list')
        );  
