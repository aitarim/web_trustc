...
class QuoteListModel {
	...
	public static function save_composite_data( $product_data, $quote_list_id ) {
		...
		// foreach($product_data as $value){
			$product = wc_get_product( $product_data['id'] );
		if ( $product->get_type() === 'composite' ) {
			update_option( 'elex_composite_data', array() );
		...
		<div class="order_data_row">
			...
			<div class="order_data_column">
				...
				<div class="address">
					...
					if ( 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) && $post->post_excerpt ) {
						echo '<p class="order_note"><strong>' . esc_attr( 'Customer provided note:', 'elex-request-a-quote' ) . '</strong> ' . nl2br( esc_html( $post->post_excerpt ) ) . '</p>';
					}
					?>
				</div>
